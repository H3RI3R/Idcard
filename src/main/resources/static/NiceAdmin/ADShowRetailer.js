...
document.getElementById('deleteRetailerForm').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        const retailerResponse = await fetch(`http://localhost:8080/api/admin/distributor/userInfo?email=${encodeURIComponent(email)}`);
        const retailerData = await retailerResponse.json();

        if (retailerResponse.ok && retailerData) {
            // Fetch token amount
            const tokenResponse = await fetch(`http://localhost:8080/api/admin/token/tokens?email=${encodeURIComponent(sessionEmail)}`);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch(`http://localhost:8080/api/admin/retailer/listAllRetailer?adminEmail=${encodeURIComponent(adminEmail)}`)
        .then(response => {
        if (!response.ok) {
