...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch(`http://localhost:8080/api/admin/retailer/list-by-creator?creatorEmail=${creatorEmail}`)
        ...
        if (data && Array.isArray(data.retailers)) {
            ...
            // Fetch tokens data
            fetch(`http://localhost:8080/api/admin/token/tokens?email=${creatorEmail}`)
                .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
...
document.getElementById('deleteRetailerForm').addEventListener('submit', function(event) {
    ...
    fetch(`http://localhost:8080/api/admin/distributor/userInfo?email=${email}`)
        .then(response => response.json())
        .then(userData => {
            if (userData && userData.email) {
                // Fetch the token amount for the user
                fetch(`http://localhost:8080/api/admin/token/tokens?email=${sessionEmail}`)
                    .then(response => response.json())
                    .then(tokenData => {
                        let tokenAmount = 0;
...
document.getElementById('confirmDelete').addEventListener('click', function() {
    ...
    fetch(`http://localhost:8080/api/admin/retailer/user-role?email=${sessionEmail}`)
        .then(response => response.json())
        .then(roleData => {
            if (roleData && roleData.role) {
                const role = roleData.role;
                // Confirm deletion
                fetch(`http://localhost:8080/api/admin/retailer/delete?email=${email}&creatorEmail=${sessionEmail}&requestingUserRole=${role}`, {
                    method: 'POST'
                })
                .then(response => {
                    if (response.ok) {
