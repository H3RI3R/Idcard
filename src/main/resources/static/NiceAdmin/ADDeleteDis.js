...
document.addEventListener('DOMContentLoaded', function () {
    ...
    deleteDistributorForm.addEventListener('submit', function (e) {
        ...
        if (email) {
            ...
            fetch(`http://localhost:8080/api/admin/distributor/userInfo?email=${encodeURIComponent(email)}`)
                .then(response => response.json())
                .then(distributor => {
                // Assuming token data is fetched here
                fetch(`http://localhost:8080/api/admin/token/count?identifier=${distributor.phoneNumber}`)
                    .then(response => response.json())
                    .then(tokenData => {
    ...
    confirmDeleteButton.addEventListener('click', function () {
        ...
        if (email) {
            ...
            fetch(`http://localhost:8080/api/admin/distributor/delete?email=${encodeURIComponent(email)}`, {
                method: 'POST'
            })
                .then(response => response.json())
                .then(data => {
