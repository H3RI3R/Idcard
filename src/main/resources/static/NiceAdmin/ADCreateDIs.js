...
document.addEventListener("DOMContentLoaded", function() {
    ...
    form.addEventListener("submit", function(event) {
        ...
        const apiUrl = `http://localhost:8080/api/admin/distributor/create?name=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}&phoneNumber=${encodeURIComponent(phoneNumber)}&designation=${encodeURIComponent(designation)}&company=${encodeURIComponent(company)}&address=${encodeURIComponent(address)}&creatorEmail=${encodeURIComponent(creatorEmail)}`;

        // Send the POST request to create the distributor
        fetch(apiUrl, {
            method: "POST",
            headers: {
            ...
            if (data.message) {
                ...
                alert("Failed to create distributor: " + data.message);
            }
        })
            .catch(error => {
            console.error("Error:", error);
            alert("An error occurred while creating the distributor.");
        });
    });
});
