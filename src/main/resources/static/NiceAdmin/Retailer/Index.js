...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('sendTokenForm').addEventListener('submit', function(event) {
        ...
        fetch(`http://localhost:8080/api/admin/token/getWalletAddress?email=${sessionEmail}`)
            .then(response => response.json())
            .then(senderData => {
            if (senderData && senderData.phoneNumber) {
                ...
                fetch(`http://localhost:8080/api/admin/distributor/userInfo?email=${recipientEmail}`)
                    .then(response => response.json())
                    .then(recipientData => {
                    if (recipientData && recipientData.email) {
                        ...
                        fetch(`http://localhost:8080/api/admin/token/count?identifier=${recipientPhoneNumber}`)
                            .then(response => response.json())
                            .then(tokenData => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const walletAddressApiUrl = `http://localhost:8080/api/admin/token/getWalletAddress?email=${userEmail}`;
    const transactionsApiUrl = 'http://localhost:8080/api/admin/token/distribution';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch(`http://localhost:8080/api/admin/retailer/list-by-creator?creatorEmail=${encodeURIComponent(creatorEmail)}`)
        .then(response => response.json())
        .then(data => {
